...
net.divideFcn = 'dividerand';  % Divide data randomlyde
net.divideMode = 'sample';  % Divide up every sample
net.divideParam.trainRatio = 60/100; % Training
net.divideParam.valRatio = 20/100; % Validation
net.divideParam.testRatio = 20/100; % Testing
...
Best_Val = valTargets;
Best_Train = trainTargets;
Val = isnan(Best_Val );
for i = 1:10
    for k = 1:282
        if Val(i,k) > 0
            Best_Val(i,k) = Best_Train(i,k);
        end
    end
end
...
TrPerformance = perform(net,Best_Val,y);
...
test_Proxy =  y .* tr.testMask{1};
        ...
        for ww = 1:size(y,1)
            for qq = 1:size(y,2)
                 AAPE_1(ww,qq) = abs((Best_Val(ww,qq)-train_Proxy(ww,qq))/Best_Val(ww,qq)); % training AAPE
            end
        end
        ...
        perflog(n,4) = TrPerformance;                 % total performance
...
fprintf('%d번째 | 구조 ==> [ %d ][ %d ] | AAPE_train ==> %4g  | AAPE_test ==> %4g \n',n,first,second,AAPE_train, AAPE_test);
% Deployment
if perflog(n,6) == min(perflog(:,6))
